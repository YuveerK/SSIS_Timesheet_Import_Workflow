...
BEGIN
    ...
    EXEC msdb.dbo.sp_add_job 
        @job_name = @jobName,
        @enabled = 1,
        @description = N'Runs the Import Timesheet Data Audit SSIS package.',
        @owner_login_name = N'YUVEER\yuvee',
        @job_id = @jobId OUTPUT;

    -- Add job step
    EXEC msdb.dbo.sp_add_jobstep 
        @job_id = @jobId,
        @step_name = N'Import timesheet files',
        @subsystem = N'SSIS',
        @command = N'/ISSERVER "\"\SSISDB\Timesheet Imports\Normalized_SSIS_Project\Import Timesheet Data Audit.dtsx\"" /SERVER YUVEER /Par "\"$ServerOption::LOGGING_LEVEL(Int16)\"";1 /Par "\"$ServerOption::SYNCHRONIZED(Boolean)\"";True /CALLERINFO SQLAGENT /REPORTING E',
